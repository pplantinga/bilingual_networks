#!/usr/bin/env python3
"""Recipe for training Transformer ASR on CommonVoice with Discrete Token Audio Inputs.
...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        audio_tokens, wav_lens = batch.audio_tokens

        embeddings = self.modules.embedding(audio_tokens)
        src = self.modules.CNN(embeddings)
    ...
    def on_fit_batch_end(self, batch, outputs, loss, should_step):
        ...
        if should_step and hasattr(self, "scheduler"):
    ...
    def on_stage_end(self, stage, stage_loss, epoch):
        ...
        if stage == sb.Stage.VALID:
            ...
            epoch_stats = {
                "epoch": epoch, "lr": lr, "steps": self.optimizer_step,
            }
...
def dataio_prepare(hparams):
    ...
    @sb.utils.data_pipeline.CachedDynamicItem.cache("feature_cache")
    @takes("id", "signal")
    @provides("audio_tokens")
    def audio_token_pipeline(uid, signal):
        """Convert continuous features into discrete tokens"""
        signal = torch.tensor(signal).unsqueeze(0).to("cuda")
        with torch.no_grad():
            feats = hparams["compute_features"](signal)
            norm_feats = hparams["normalize"](feats)
            discrete_feats = hparams["quantizer"](norm_feats)
        return discrete_feats.squeeze(0)
    ...
    pipelines = [path_pipeline, audio_pipeline, word_pipeline, audio_token_pipeline]
    ...
    for dataset in datasets.values():
        ...
        dataset.set_output_keys(["id", "audio_tokens"] + token_keys)
